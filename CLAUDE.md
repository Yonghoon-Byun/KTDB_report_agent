...
[KTDB 원본 zip 3개]                ← docs/PRD_DB-플랫폼-Azure-전환.md 참조
        ...
        ↓ sql/db_schema_smr.sql            ← 9 테이블 + 21 파티션 + 60 컬럼 COMMENT
...
### 1. 신규 — 수도권 (`smr_*`, `sql/db_schema_smr.sql`)
...
### 2. Legacy — 전국 (`sql/db_schema.sql`, Supabase 잔여)
...
1. **DB 분양**은 `docs/테넌트_DB_분양_절차.md` 그대로. Step 4 직전 `current_database()` 1줄 검증 의무 (§7 사고 재발 방지).
...
| `sql/db_schema_smr.sql` 적용 | 개발자 | ✅ 30 테이블 + 60 COMMENT |
...
KTDB_report_agent/
├── CLAUDE.md                       # 본 파일
├── streamlit_app.py                # 메인 앱 (자연어 분석 인터페이스)
├── db_env.py                       # DB 자격증명·연결 헬퍼 (Streamlit + CLI 공용)
├── sync_smr.py                     # zip → ktdb 단계별 적재 (zones/socio/od2023/od_all)
├── sync_sheets_to_db.py            # legacy 시트 → Supabase 동기화 (보존용)
├── requirements.txt                # Python 의존성
├── 질문.md                          # 테스트 질의 9건 (수도권 데이터 범위 기준)
├── .gitignore                      # .env, secrets.toml, .tmp_extract/, 원본자료/, .claude/, dist/, __pycache__/
...
├── sql/                            # SQL 스키마 정의
│   ├── db_schema.sql               # legacy 전국 250존 (Supabase 잔여)
│   └── db_schema_smr.sql           # 수도권 1310 TAZ (9 부모 + 21 파티션 + 60 COMMENT)
├── scripts/                        # 운영 스크립트
...
├── docs/                           # 문서
│   ├── PRD_DB-플랫폼-Azure-전환.md   # 의사결정 (Azure 시나리오)
│   ├── 컬럼매핑표.md                 # 원본 ↔ DB 컬럼 마스터
│   ├── 테넌트_DB_분양_절차.md         # GIS 팀 표준 분양 절차 (§7 ktdb 사례)
│   ├── 사용자-설치-가이드.md          # 사내 분배용 (Python·secrets.toml·streamlit run)
│   ├── 비개발자-수정-가이드.md
│   ├── 왜-데이터-연동이-안되는가.md   # 옛 진단 (시트 OAuth 시대)
│   ├── 수정사항 분석.md
│   └── 참고자료/                    # KTDB 원본 안내 문서 (hwpx, pdf)
├── dist/                           # 사내 분배 패키지 (gitignored, 절대 커밋 금지)
│   ├── secrets.toml                # ktdb_viewer R/O 자격증명 — 사용자에게 전달용
│   └── KTDB_분석에이전트.zip        # 분배본 (코드 + secrets + 가이드)
├── 원본자료/                        # KTDB zip 3개 989MB (gitignored)
└── .tmp_extract/                   # zip 임시 추출본 (gitignored, 자동 생성·삭제)
```
...
- **2026-05-06**: **Azure Postgres ktdb 마이그레이션 + 수도권 1310 TAZ 신규 적재 준비**.
  - GIS 팀 인프라 공유: 같은 `geo-spatial-hub` 서버에 ktdb 테넌트 분양 (`docs/테넌트_DB_분양_절차.md` 표준 따름).
  - 자동화 스크립트 `scripts/provision_ktdb.py` 작성 + 실제 분양 완료. 검증 6항목 PASS.
  - `sql/db_schema_smr.sql` 작성 + 적용: 9 부모 + 21 파티션 (year list) + 60 컬럼 COMMENT.
...
- **2026-05-07 (정리)**: **개발 폴더 재구성 — 루트 정돈**.
  - 임시·캐시 파일 삭제: `1.png`, `2.png`, `__pycache__/`, `.tmp_extract/`, `streamlit_app_old.py` (git history에 보존).
  - SQL 파일 분리: `db_schema.sql`, `db_schema_smr.sql` → `sql/` 신규 폴더.
  - 분양 절차 문서 이동: `테넌트_DB_분양_절차.md` → `docs/` 안으로.
  - KTDB 참고자료(hwpx, pdf) → `docs/참고자료/` 신규 폴더.
  - psql 명령 갱신: `psql ... -f sql/db_schema_smr.sql` (sql/ 경로 반영).
  - git mv로 rename 추적, 코드 import 영향 0.
